...
const string OpenCLHelpers::getErrorMessage(cl_int error)
{
  ...
  default: return "Unknown OpenCL error " + to_string(error);
  }
}

void OpenCLHelpers::checkErrors(cl_int error, const char* file, const char* func, int line) {
  if(error != 0)
    throw runtime_error(string("OpenCL error at ") + file + ", func " + func + ", line " + to_string(line) + ", error " + getErrorMessage(error));
}
...
cl_program OpenCLHelpers::compileProgram(const string& name, cl_context context, const vector<cl_device_id>& devices, const string& str, const string& options) {
  ...
  if(err != 0) {
    ...
    for(int i = 0; i<devices.size(); i++) {
      ...
      CHECK_ERR(err2);
      s += "BUILD LOG FOR " + name + " ON DEVICE " + to_string(i) + "\n";
      s += buf.data() + string("\n");
    }
...
vector<DeviceInfo> DeviceInfo::getAllDeviceInfosOnSystem() {
  ...
  for(int platformIdx = 0; platformIdx < numPlatforms && numDevicesTotal < deviceIds.size(); platformIdx++) {
    ...
    cout << "Found OpenCL Platform " + to_string(platformIdx) + ": " + desc << endl;
    ...
    if(err == CL_DEVICE_NOT_FOUND) {
        cout << "Found 0 device(s) on platform " + to_string(platformIdx) + " with type CPU or GPU or Accelerator, skipping" << endl;;
      continue;
    }
    ...
    cout << "Found " + to_string(numDevices) + " device(s) on platform " + to_string(platformIdx) + " with type CPU or GPU or Accelerator" << endl;
  }
  ...
  for(int gpuIdx = 0; gpuIdx<numDevicesTotal; gpuIdx++) {
    ...
    cout <<
        "Found OpenCL Device " + to_string(gpuIdx) +
        ": " + name + " (" + vendor + ")" + " (score " +
        to_string(defaultDesirability) + ")"
        << endl;
    ...
    uniqueDeviceNamesToUse()
{
  ...
  for(size_t i = 1; i<gpuIdxsToUse.size(); i++) {
    if(gpuIdxsToUse[i-1] == gpuIdxsToUse[i])
      throw StringError("Requested gpuIdx/device more than once: " + to_string(gpuIdxsToUse[i]));
  }
  ...
  for(size_t i = 0; i<gpuIdxsToUse.size(); i++) {
    ...
    if(gpuIdx < 0 || gpuIdx >= allDeviceInfos.size()) {
      ...
      throw StringError(
        "Requested gpuIdx/device " + to_string(gpuIdx) +
        " was not found, valid devices range from 0 to " + to_string((int)allDeviceInfos.size() - 1)
      );
    }
  ...
  for(size_t i = 0; i<gpuIdxsToUse.size(); i++) {
    ...
    string message =
      "Using OpenCL Device " + to_string(gpuIdxsToUse[i]) + ": " + device->info.name +
      " (" + device->info.vendor + ") " +
...
const InitializedDevice* DevicesContext::findGpuExn(int gpuIdx) const {
  ...
  throw StringError("BUG? Attempted to create ComputeHandle for a gpuIdx that was not part of the DevicesContext: " + to_string(gpuIdx));
}
